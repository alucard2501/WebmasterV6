...
(function ($) {
    $.extend($.fn, {
        ...
        */
        validateOnBlur: function (language, settings) {
            ...
            $el.find('textarea,input,select').blur(function () {
                ...
                $('#Span_TipsUsername').css('display', 'none');
            });
        ...
        */
        doValidate: function (language, settings, attachKeyupEvent) {
            ...
            var config = {
                ...
                serviceValidateUserNameIsUsed: '../services/get_data.aspx?tips=CHECK_USERNAME_IS_USED',
                serviceValidateCheckCode: '../services/get_data.aspx?tips=CHECK_CHECK_CODE'
            };
            ...
            $element.parent().find('.tipsOK').remove();
            $element.parent().find('.tipsError').remove();
            ...
            if (!jQueryFormUtils.ignoreInput($element.attr('name'), elementType, config)) {
                ...
                if (validation === true) {
                    ...
                    //$element.parent().append('<span class="tipsOK"></span>');
                } else {
                    ...
                    if (elementErrMsgObj != null) {
                        ...
                        //$element.parent().append('<span class="tipsError"></span>').append('<span class="jquery_form_error_message">' + validation + '</span>');
                        $element.parent().append('<span class="tipsError">' + validation + '</span>');
                    }
                    ...
                    //启动等待
                    asyncValidating($element, config, language, elementErrMsgObj);
                }
            }
        ...
        */
        validate: function (language, settings) {
            ...
            */
            ...
            $form.find('input,textarea,select').each(function () {
                ...
                $el.css('border-color', jQueryFormUtils.defaultBorderColor);
                $el
                .removeClass(config.errorElementClass)
                .parent()
                    .find('.jquery_form_error_message').remove();
                $el.parent().find('.tipsOK').remove();
                $el.parent().find('.tipsError').remove();
                ...
                if (!jQueryFormUtils.ignoreInput($el.attr('name'), elementType, config)) {
                    ...
                    // inputs, textareas and select lists
                        ...
                        if (valid !== true) {
                            ...
                            $el.addClass(config.errorElementClass);
                            //$el.parent().append('<span class="tipsError"></span>').append('<span class="jquery_form_error_message">' + valid + '</span>');
                            $el.parent().append('<span class="tipsError">' + valid + '</span>');

                            if (config.borderColorOnError !== '') {
                                $el.css('border-color', config.borderColorOnError);
                            }
        ...
        //提交Form
        submitValidate: function (action, language, settings) {
            var $form = $(this);
            if ($form.validate(language, settings)) {
                this.action = action;
                this.submit();
            } else if (jQueryFormUtils.asyncValidateStatus == "Validating") {
...
function addErrorMessage($element, errMsg) {
    var config = {
        errorElementClass: 'error', // Class that will be put on elements which value is invalid
        borderColorOnError: 'red' // Border color of elements which value is invalid, empty string to not change border color
    };
    ...
    $element.parent().find('.tipsError').remove();
    $element.addClass(config.errorElementClass);
    $element.parent().append('<span class="tipsError"></span>').append('<span class="jquery_form_error_message">' + errMsg + '</span>');

    if (config.borderColorOnError !== '') {
        $element.css('border-color', config.borderColorOnError);
    }
}
function asyncValidating($element, config, language, elementErrMsgObj) {
    
    if (jQueryFormUtils.asyncValidateStatus == "Validating") {
        _setTimeout(asyncValidating, 2000, $element, config, language, elementErrMsgObj);

        //setTimeout(asyncValidating($element, config, language, elementErrMsgObj), 2000);
    }
    if (jQueryFormUtils.asyncValidateStatus == "Fault") {
        var validationErrorMsg = $element.attr(config.validationErrorMsgAttribute) || language.userNameIsUsed;
        $element.parent().find('.jquery_form_error_message').remove();
        if (elementErrMsgObj != null) {
            elementErrMsgObj.innerHTML = validationErrorMsg;
        } else { // use regular span append
            $element.parent().append('<span class="jquery_form_error_message">' + validationErrorMsg + '</span>');
        }
    }
    if (jQueryFormUtils.asyncValidateStatus == "Success") {
        if (config.borderColorOnError!=="") {
            $element.css('border-color', jQueryFormUtils.defaultBorderColor);
        }
        $element
...
//我加的一个全局变量asyncValidateStatus
//状态有四
//asyncValidateStatus="None" ----未启动异步验证
//asyncValidateStatus="Validating" ----异步验证中
//asyncValidateStatus="Fault" ----异步验证失败
//asyncValidateStatus="Success" ----异步验证成功
jQueryFormUtils.asyncValidateStatus = "None";
/**
 * Static variable for holding default border color on input
 */
jQueryFormUtils.defaultBorderColor = "#C9C9C9";
...
jQueryFormUtils.validateEmailRequired = function (email) {
...
* @return {Boolean}
*/
jQueryFormUtils.validateUserNameIsUsed = function (username, service) {
    if (jQueryFormUtils.asyncValidateStatus != 'Validating') {
        jQueryFormUtils.asyncValidateStatus = 'Validating';
        //挂起一个异步线程
        //    $.get(VALIDATE + '?tips=CHECK_USERNAME_IS_USED', { Text_UserName: username }, function (response, status, xhr) {
        //        alert(response);
        //    }, "text");
        $.get(service, { Text_UserName: username }, function (response, status, xhr) {
            //alert(response);
            if (response == "True") {
                jQueryFormUtils.asyncValidateStatus = 'Success';
            }
            if (response == "False") {
                jQueryFormUtils.asyncValidateStatus = 'Fault';
            }
        }, "text");
    }
};
...
jQueryFormUtils.validate = function (value, el, language, config, form) {
    ...
    if (typeof validationRules != 'undefined' && validationRules !== null) {
        ...
        // Email_required
        if (validationRules.indexOf('validate_email_required') > -1 && !jQueryFormUtils.validateEmailRequired(value)) {
        ...
        // Float
            ...
            if (value != "") {
                if (!jQueryFormUtils.validateInteger(value)) {
        ...
        // username not used
        if (validationRules.indexOf('validate_username_is_used') > -1) {
            jQueryFormUtils.validateUserNameIsUsed(value, config.serviceValidateUserNameIsUsed);
            return "验证中";
        }
        // confirmation
...
jQueryFormUtils.LANG =  {
    ...
    notConfirmed : '密码与确认密码不一致',
    badDomain : 'Incorrect domain value',
    badUrl : 'Incorrect url value',
    badFloat: '请输入小数',
    badCustomVal : '您输入的格式不正确',
    badInt: '请输入整数',
    badSecurityNumber : 'Your social security number was incorrect',
    badUKVatAnswer : 'Incorrect UK VAT Number',
    badNumberOfSelectedOptionsStart : 'You have to choose at least ',
    badNumberOfSelectedOptionsEnd : ' answers',
    userNameIsUsed : 'Username is used,please use another one',
    badCheckCode: 'Incorrect check code'
};
