...
function loadDoucument() {
    ...
    $('#form1').attr('action', 'SAVE_SYS_FILE_2');
        ...
        success: function (response, status, xhr) {
            if (response.status == "SUCCESS") {
                ...
                for (var i = 1; i <= response.records.length; i++) {
                    ...
                    str = str + '<td>' + record.fileType + '</td>';
                    str = str + '<td>' + record.name + '</td>';
                    str = str + '<td>' + record.path + '</td>';
                    str = str + '<td>' + bytesToSize(record.size) + '</td>';
...
function loadFileType() {
    var s = $("#Select_FileType");
    s.find('option').remove();

    var s2 = $("#SelectFilter_FileType");
...
function btnSearch() {
    ...
    if ($("#SelectFilter_FileType").val() != "" && $("#SelectFilter_FileType").val() != null) _constr = _constr + " AND " + $("#SelectFilter_FileType").attr("fieldname") + "=" + $("#SelectFilter_FileType").val();
