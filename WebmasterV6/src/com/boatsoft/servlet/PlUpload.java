...
//        }  
        try{
        	...
        	String str_is_excel=request.getParameter("isexcel");
        	String excelId=request.getParameter("excelId");
        	boolean is_excel_import=(str_is_excel.equals("1"));
	        ...
	        //获取文件分类路径
	        String pathFileType1="", pathFileType2="";
	        String sql="SELECT * FROM t_sys_file_type WHERE id="+ fileTypeId;
			JSONArray rs=dao.fillRS(sql,s.config);
			if(rs.length()>0){
				JSONObject row=rs.getJSONObject(0);
				pathFileType1=row.getString("uploadPath");
			}
			if(pathFileType1!=""){
				pathFileType2="/"+pathFileType1;
				pathFileType1=pathFileType1+"/";
			}
			HttpSession session = request.getSession();
        	UploadFileInfo fi=uploadFiles(request,response,s.config.UPLOAD_PATH + "/file/upload" +pathFileType2);
        	int countPreviewPic=0;
        	if(!is_excel_import){
        		if(fi.FileNameNew.endsWith("docx") || fi.FileNameNew.endsWith("doc")){
            		String fullPath = s.config.UPLOAD_PATH + "/file/upload" +pathFileType2+"/"+fi.FileNameNew;
            		countPreviewPic=BasicFunction.doc2jpg(fullPath);
            	}
            	if(fi.FileNameNew.endsWith("xlsx") || fi.FileNameNew.endsWith("xls")){
            		String fullPath = s.config.UPLOAD_PATH + "/file/upload" +pathFileType2+"/"+fi.FileNameNew;
            		countPreviewPic=BasicFunction.xls2jpg(fullPath);
            	}
            	if(fi.FileNameNew.endsWith("pdf")){
            		String fullPath = s.config.UPLOAD_PATH + "/file/upload" +pathFileType2+"/"+fi.FileNameNew;
            		countPreviewPic=BasicFunction.pdf2jpg(fullPath);
            	}
        	}
        	...
        	//System.out.println(fi.FileNameSrc + "," + fi.FileNameNew + ","+fi.FileExt);
        	//导入excel相关的工作
//        	if(is_excel_import){
//        		File file=new File(s.config.UPLOAD_PATH + "/file/upload" +pathFileType2 + "/" + fi.FileNameNew);
//        		ArrayList<ArrayList<Object>> row_list=ExcelUtil.readExcel(file);
//        		importExcel(row_list, Integer.parseInt(excelId),dao,s.config);
...
//        	}
	...
	public UploadFileInfo uploadFiles(HttpServletRequest request,HttpServletResponse response, String rePath) throws Exception{
		...
		if(session.getAttribute("size")!=null){
			...
			size=Long.parseLong(session.getAttribute("size").toString());
		}
